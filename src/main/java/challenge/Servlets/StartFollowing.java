...
public class StartFollowing {
    ...
    @RequestMapping(value = "/start-follow", method = RequestMethod.GET)
    public String startFollowing(@AuthenticationPrincipal final UserDetails user, HttpServletRequest request) throws JsonProcessingException {
        ObjectMapper om = new ObjectMapper();
        UserFollowHandler follow = new UserFollowHandler();
        try {
            follow.startFollowing(user.getUsername(), request.getParameter(ChallengeConstants.USER_TO_FOLLOW));
        }
        catch (Exception e){
